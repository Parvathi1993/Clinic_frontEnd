...
import IpShowPrint from './IpShowPrint'
...
const IpBillingmain = () => {
    ...
    const updatedoa = useCallback((e) => {
        setDoa(e.target.value)
    }, [])

    const updatedod = useCallback((e) => {
    ...
    const AddProcedure = useCallback(() => {
        if (procedure === 0) {
            warningNotify("Please Select Any Procedure and count")
        } else {
            const newData = {
    ...
    }, [procedure, produrname, produrname, rate, count])

    let sumProcedureRate = dataPost.reduce((sum, obj) => sum + obj.ip_rate_total, 0);
    ...
    const postData = useMemo(() => {
        return {
            ...
            ip_bill_date: format(new Date(), "yyyy-MM-dd hh:mm:ss"),
            ip_bill_amount: sumProcedureRate,
            ip_bill_payment_mode: radiovalue === '2' ? 2 : radiovalue === '3' ? 3 : 1,
            admission_date: format(new Date(doa), "yyyy-MM-dd hh:mm:ss"),
            discharge_date: format(new Date(dod), "yyyy-MM-dd hh:mm:ss")
        }
    }, [pateintid, sumProcedureRate, radiovalue, doa, dod])

    const submit = useCallback(() => {
        const insertBillMast = async (postVisitMast) => {
            const result = await axioslogin.post('/Billing/ipBillinsert', postVisitMast);
            return result.data
        }
        ...
        const InserBillndetl = async (insertid) => {
            const postdataDetl = dataPost && dataPost.map((val, index) => {
                return {
                    ip_bill_slno: insertid,
                    ip_procedure_slno: val.procedure,
                    ip_procedure_rate: val.procedureRate,
                    ip_bill_proc_slno: index + 1,
                    ip_count: val.ip_count,
                    ip_rate_total: val.ip_rate_total
                }
            })
            const result = await axioslogin.post('/Billing/IpBillDetailsInsert', postdataDetl);
    ...
    }, [postData, dataPost, pateintid, doa, dod])
    ...
    return (
        <Box sx={{ width: "100%", p: 5 }}>
            ...
            {modal === 1 ? <IpShowPrint open={modalFlag} lastVisitId={lastVisitId} reset={reset}
                patient={patient} pateintid={pateintid} sumProcedureRate={sumProcedureRate} admission_date={doa}
                discharge_date={dod} /> : null}
            <ToastContainer />
            <Paper sx={{
                width: '100%',
                mt: 0.8
                ...
                <Box sx={{
                    ...
                    <Box sx={{ width: '100%', display: 'flex', justifyContent: "center" }}>
                        <Typography level='body-md' fontWeight='lg' >IP Billing</Typography>
                    </Box>
                    ...
                    <Box sx={{ width: '80%', display: 'flex', pt: 2.5, margin: 'auto ', pl: 10, flexDirection: "column" }}>
                        ...
                        <Box sx={{ width: "100%", display: 'flex', flexDirection: "row", pt: 1 }}>
                            <Box sx={{ width: "20%", pt: 0.5, }}>
                                <Typography sx={{ fontSize: 13, fontFamily: 'sans-serif', fontWeight: 550 }} >Date of Admission</Typography>
                            </Box>

                            <Box sx={{ width: "25%", pt: 0.5, }}>
                                <CustomInput placeholder={"Date of Admission"}
                                    //type="date-time"
                                    type="datetime-local"
                                    size="sm"
                                    slotProps={{
                                        input: {
                                            max: format(new Date(), "yyyy-MM-dd hh:mm:ss")
                                        },
                                    }}
                                    name="doa"
                                    value={doa}
                                    handleChange={updatedoa} />
                            </Box>
                            <Box sx={{ width: "5%", pt: 0.5, }}>
                            </Box>
                            <Box sx={{ width: "20%", pt: 0.5, }}>
                                <Typography sx={{ fontSize: 13, fontFamily: 'sans-serif', fontWeight: 550 }} >Date of Discharge</Typography>
                            </Box>
                            ...
                            <Box sx={{ width: "25%", pt: 0.5, }}>
                                <CustomInput placeholder={"Date of Admission"}
                                    ...
                                    slotProps={{
                                        input: {
                                            min: format(new Date(doa), "yyyy-MM-dd hh:mm")
                                        },
                                    }}
                                    name="dod"
                                    value={dod}
                                    handleChange={updatedod} />
                            </Box>
                        </Box>
                        ...
                        <Box sx={{ width: "100%", display: 'flex', flexDirection: "column", pt: 1, pb: 1.5 }}>
                            ...
                            <Box sx={{ width: "100%", display: 'flex', flexDirection: "row", pt: 1 }}>
                                ...
                                <Box sx={{ pl: 2, width: "15%" }}>
                                    <Typography sx={{ fontSize: 13, fontFamily: 'sans-serif', fontWeight: 550, textAlign: 'center' }} >Count</Typography>
                                </Box>
                            </Box>
                            <Box sx={{ width: "100%", display: 'flex', flexDirection: "row", pt: 1 }}>
                                <Box sx={{ pl: 2, width: "5%" }}>
                                            ...
                                            <Table stickyHeader>
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th style={{ width: '40%', align: "center" }}>Procedure Name</th>
                                                        <th style={{ width: '30%', align: "center" }}>Procedure Rate</th>
                                                        <th style={{ width: '20%', align: "center" }}>Count</th>
                                                        <th style={{ width: '30%', align: "center" }}>Total Amount</th>
...
export default memo(IpBillingmain)